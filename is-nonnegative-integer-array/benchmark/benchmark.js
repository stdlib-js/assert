...
function main() {
	...
	for ( i = min; i <= max; i++ ) {
		...
		f = createBenchmark( isNonNegativeIntegerArray, len, false );
		bench( pkg+':len='+len, f );

		f = createBenchmark( isNonNegativeIntegerArray.primitives, len, true );
		bench( pkg+':primitives:len='+len, f );

		f = createBenchmark( isNonNegativeIntegerArray.objects, len, false );
