...
function main() {
	...
	for ( i = min; i <= max; i++ ) {
		...
		f = createBenchmark( isSafeIntegerArray, len, false );
		bench( pkg+':len='+len, f );

		f = createBenchmark( isSafeIntegerArray.primitives, len, true );
		bench( pkg+':primitives:len='+len, f );

		f = createBenchmark( isSafeIntegerArray.objects, len, false );
