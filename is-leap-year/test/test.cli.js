...
tape( 'the command-line interface prints out whether the current year corresponds to a leap year in the Gregorian calendar', opts, function test( t ) {
	var cmd;
	var str;
...
tape( 'the command-line interface tests if an input argument corresponds to a leap year in the Gregorian calendar', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		'-e',
...
tape( 'the command-line interface supports use as a standard stream', opts, function test( t ) {
	...
	function done( error, stdout, stderr ) {
		if ( error ) {
			...
			expected = [ true, false, false, false, true ].join( '\n' )+'\n';
