...
tape( 'the command-line interface tests whether a value is a relative URI', opts, function test( t ) {
	var opts;
	var cmd;
	...
	cmd = [
		EXEC_PATH,
		'-e',
		'"process.stdin.isTTY = true; process.argv[ 2 ] = \'google.com\'; require( \''+fpath+'\' );"'
	];
	opts = {};
...
tape( 'the command-line interface supports use as a standard stream', opts, function test( t ) {
	var cmd = [
		'printf "http://www.ietf.org/rfc/rfc2396.txt\nbeep"',
		'|',
		EXEC_PATH,
		fpath
	];
...
tape( 'the command-line interface supports specifying a custom delimiter when used as a standard stream (string)', opts, function test( t ) {
	var cmd = [
		'printf "http://www.ietf.org/rfc/rfc2396.txt\tbeep"',
		'|',
		EXEC_PATH,
		fpath,
		'--split \'\t\''
