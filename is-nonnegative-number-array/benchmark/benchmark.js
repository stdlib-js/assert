...
function main() {
	...
	for ( i = min; i <= max; i++ ) {
		...
		f = createBenchmark( isNonNegativeNumberArray, len, false );
		bench( pkg+':len='+len, f );

		f = createBenchmark( isNonNegativeNumberArray.primitives, len, true );
		bench( pkg+':primitives:len='+len, f );

		f = createBenchmark( isNonNegativeNumberArray.objects, len, false );
