...
function createBenchmark( fcn, len, primitives ) {
	...
	if ( primitives ) {
		...
		for ( i = 0; i < len; i++ ) {
			x.push( new Number( randu() ) ); // eslint-disable-line no-new-wrappers
		}
	}
	return benchmark;
...
function main() {
	...
	for ( i = min; i <= max; i++ ) {
		...
		f = createBenchmark( isProbabilityArray, len, false );
		bench( pkg+':len='+len, f );

		f = createBenchmark( isProbabilityArray.primitives, len, true );
		bench( pkg+':primitives:len='+len, f );

		f = createBenchmark( isProbabilityArray.objects, len, false );
