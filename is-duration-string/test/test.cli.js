...
tape( 'the command-line interface tests whether an input argument is a duration string', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		'-e',
		'"process.stdin.isTTY = true; process.argv[ 2 ] = \'1h1m1s\'; require( \''+fpath+'\' );"'
	];
...
tape( 'the command-line interface supports providing an alternative delimiter when used as a standard stream (space)', opts, function test( t ) {
	...
	cmd = [
		'printf "1h2m3s 1h2m3sX 1h2m3sX 1h2m3s"',
		'|',
		EXEC_PATH,
		fpath,
		'--split=" "'
...
tape( 'the command-line interface supports providing an alternative delimiter when used as a standard stream (regular expression)', opts, function test( t ) {
	...
	function done( error, stdout, stderr ) {
		if ( error ) {
			...
			expected = [ true, false, true, true ].join( '\n' )+'\n';
			t.strictEqual( stdout.toString(), expected, 'expected value' );
			t.strictEqual( stderr.toString(), '', 'does not print to `stderr`' );
		}
		t.end();
