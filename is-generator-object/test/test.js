...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof isGeneratorObject, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function returns `false` if not provided a generator object', function test( t ) {
	var values;
	var i;
	values = [
		'5',
	...
	for ( i = 0; i < values.length; i++ ) {
		t.strictEqual( isGeneratorObject( values[i] ), false, 'returns expected value' );
	}
	t.end();
});
